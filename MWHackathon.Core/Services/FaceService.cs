...
namespace MWHackathonHarvester.Services
{
  ...
  public class Faces
  {
    public Faces(string json, string url, int? width, int? height)
      : this(JObject.Parse(json), url, width, height)
    {
    }
    ...
    public Faces(JObject json, string url, int? width, int? height)
    {
      this.Url = url;
      ...
      if (height.HasValue)
        ...
        this.Height = Convert.ToInt32(JSONService.GetText(photo, "height"));
    }
